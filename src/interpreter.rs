...
impl Interpret for Expr {
    fn interpret(
        ...
        match *self {
            ...
            Expr::Get(ref g) => match g.instance.interpret(environment, scope_resolver) {
                Ok(Value::Instance(ref instance)) => instance.get(g.property),
                Ok(v) => Err(RuntimeError::NotAnInstance(v.clone())),
                e => e,
            },
            Expr::Set(ref s) => match s.instance.interpret(environment, scope_resolver) {
    ...
    fn execute(
        ...
        match *self {
            ...
            Statement::Print(ref e) => {
                e.interpret(environment, scope_resolver).map(|value| {
